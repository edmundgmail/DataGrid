...
public final class KafkaUtils {
  ...
  public static void maybeCreateTopic(String zkServers,
    ...
    try {
      if (AdminUtils.topicExists(zkUtils, topic)) {
        ...
        try {
          AdminUtils.createTopic(zkUtils, topic, partitions, 1, topicProperties, RackAwareMode.Safe$.MODULE$);
  ...
  public static Map<Pair<String,Integer>,Long> getOffsets(String zkServers,
    ...
    try {
      ...
      partitions.forEach(partition -> {
        ...
        offsets.put(Pair.of(topic, Integer.parseInt(partition.toString())), offset);
      });
    } finally {
      zkUtils.close();
    }
  ...
  public static void setOffsets(String zkServers,
    ...
    try {
      offsets.forEach((topicAndPartition, offset) -> {
        ZKGroupTopicDirs topicDirs = new ZKGroupTopicDirs(groupID, topicAndPartition.getLeft());
        int partition = topicAndPartition.getRight();
        String partitionOffsetPath = topicDirs.consumerOffsetDir() + "/" + partition;
  ...
  public static void fillInLatestOffsets(Map<Pair<String,Integer>,Long> offsets,
    ...
    offsets.keySet().forEach(topicPartition -> {
      TopicAndPartition tAndP = new TopicAndPartition(topicPartition.getLeft(), topicPartition.getRight());
  ...
  private static long getOffset(OffsetResponse response, Pair<String,Integer> topicPartition) {
    String topic = topicPartition.getLeft();
    int partition = topicPartition.getRight();
